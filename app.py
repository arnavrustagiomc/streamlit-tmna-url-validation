...
def main():
    ...
    standard_page = st.Page("app.py", title="Standard Validator", default=True)
    ai_page = st.Page("app_ai.py", title="AI Validator")
    selected_page = st.navigation([standard_page, ai_page])

    if selected_page == standard_page:
        run_standard_validator()
    else:
        selected_page.run()
